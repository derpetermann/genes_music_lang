...
# Connect to DB 
secret_db_login <- "PG:host='limits.geo.uzh.ch' dbname='limits-db' port=5432 user='contact_zones' password='letsfindthemcontactzones'"
geo_mem <- readRDS('../data/preprocessed/geo_mem.RDS')
rda_sp_all <- readRDS('../data/preprocessed/rda_sp_all.RDS')
rda_all <- readRDS('../data/preprocessed/rda_all.RDS')
geo_polygons <- readRDS('../data/preprocessed/geo_polygons.RDS')
geo_random_points <-readRDS('../data/preprocessed/geo_random_points.RDS')
```
...
trim_data <- function(data.list, trim.to=siberia_metadata_all, extra.coverage=.7) {
 ...
 #' @param trim.to contains the ids of those languuages that are retained after trimming  
...
```{r map, message=FALSE, warning=FALSE,  fig.width=7, fig.height=6, fig.cap="\\label{fig:figs}Geographic locations of the thirteen languages.", fig.path='figures/', dev='cairo_pdf'}
...
```{r dbmem, message=FALSE, warning=FALSE, eval=FALSE}
...
# Retain the first k PCs/Pcos which togheter account for 80% of the variance 
var_th <- 0.8
...
```{r heat_map_genetics, fig.cap="\\label{fig:figs} Heat plot of the first four PCos (normalized) for Genetics for each of the 13 populations.", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7}

heat_map_pc(factors$genetics, type="PCo")
```


```{r heat_map_music, fig.cap="\\label{fig:figs} Heat plot of the first five PCos (normalized) for Music for each of the 13 populations.", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7}

heat_map_pc(factors$music, type="PCo")
```

```{r heat_map_grammar, fig.cap="\\label{fig:figs} Heat plot of the first six PCs (normalized) for Grammar for each of the 13 populations.", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7}

heat_map_pc(factors$grammar, type="PC")
```

```{r heat_map_phonology, fig.cap="\\label{fig:figs} Heat plot of the first six PCs for Phonology (normalized) for each of the 13 populations.", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, }
...
Redundancy Analysis (RDA) extracts the variation in a set of explanatory variables that can be explained by a set of response variables  [@legendre_2012]. In our case the response and explanatory variables comprise the principle components (coordinates) of a factor, e.g. the explanatory variable may comprise the PCs of grammar and the response those of phonology. RDA carries out a multiresponse multiple linear regression, i.e. regression of multiple response variables on multiple explanatory variables [@van_den_wollenberg_1977]. The results generated through this regression are then "subject to "summarized" in aprincipal components analysis. 

## Pearson Correlation Coefficient 

Prior to RDA, we compute the simple Pearson correlation coefficient between all individual principal components/ prinicipal coordinates. We visualize the correlation coefficient and the significance for all pairs.
...
We carry out a simple RDA on genetics, grammar, music and phonology, where each of the factors is used as either the response or the explanatory variable in turns. This yields twelve RDA pairs for which we report the coefficient of determination (adjusted R squared) and the significance of the correlatation determined by an ANOVA like permutation test.  

```{r rda, eval=FALSE}

# All possible combinations for which we perfom RDA
...
```{r rda plot, fig.cap="\\label{fig:figs} Redundancy Analysis: Variance in the response explained by each explanatory variable", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7}
...
```{r density_plot 1, fig.cap="\\label{fig:figs} Partial RDA of Genetics (explanatory variable) and Grammar (response)", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7}
...
```{r density_plot 2, fig.cap="\\label{fig:figs} Partial RDA of Genetics (explanatory variable) and Phonology (response)", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, message=FALSE, warning=FALSE}

# Genetics --> Phonology
spatial_density_plot(rda_sp_all$genetics_phonology, "r2_adj")
```

```{r density_plot 3, fig.cap="\\label{fig:figs} Partial RDA of Grammar (explanatory variable) and Phonology (response)", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, message=FALSE, warning=FALSE}

# Grammar --> Genetics
spatial_density_plot(rda_sp_all$grammar_genetics, "r2_adj")
```

```{r density_plot 4, fig.cap="\\label{fig:figs} Partial RDA of Grammar (explanatory variable) and Music (response)", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7,  message=FALSE, warning=FALSE}

# Grammar --> Music
spatial_density_plot(rda_sp_all$grammar_music, "r2_adj")
```

```{r density_plot 5, fig.cap="\\label{fig:figs} Partial RDA of Grammar (explanatory variable) and Phonology (response)", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, message=FALSE, warning=FALSE}

# Grammar --> Phonology
spatial_density_plot(rda_sp_all$grammar_phonology, "r2_adj")
```

```{r density_plot 6, fig.cap="\\label{fig:figs} Partial RDA of Phonology (explanatory variable) and Grammar (response)", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7, message=FALSE, warning=FALSE}

# Phonology --> Grammar
spatial_density_plot(rda_sp_all$phonology_grammar, "r2_adj")
```

##  Plotting locations with low adjusted R squared  

Spatial neighborhood or unlikely to explain the observed relationship between factors if the adjusted R squared for all spatial samples is well above zero and if it does not overlap with the adjusted R squared for random permutations.To further rule out possible influence of space on the observed correlation we plot those location samples for which the adjusted R-squared is particularily low, i.e. in the 0.05 percentile. If all locations with a low R squared cluster in distinct regions of the polygons this gives rise to the assumption that under a particular pattern of spatial neighborhood, spatial neighborhood explains the observed relationship between two factors. If the spatial locations distribute randomly, however, the low adjusted-R squared values are random negative outliers. 

```{r low_rda 1, fig.cap="\\label{fig:figs} Point samples used for removing the influence of space in the partial RDA between Grammar (explanatory variable) and Phonology (response) with a low R squared", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7}
...
```{r low_rda 2, fig.cap="\\label{fig:figs} Point samples used for removing the influence of space in the partial RDA between Genetics (explanatory variable) and Grammar (response) with a low R squared", fig.path='figures/', dev='cairo_pdf', fig.width=10, fig.height=7}
