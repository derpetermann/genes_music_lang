...
trim_data <- function(data.list, trim.to=siberia_metadata_all, extra.coverage=.7) {
 ...
 #' @param trim.to contains the id of all laguages which should remain after trimming  
...
### Visualizations (Themas and Functions)

```{r}

# Thema for overview maps
theme_map <- function(...) {
  theme_minimal() +
  theme(
...
plot_language_map <- function(lang_poly, lang_labels, languages) {
...
We read the language polygons from a geo-database. Moreover, we also retrieve 10,000 samples of point locations taken randomly from the language polygons. 
...
We rescale the PCoA components. 
...
First, we plot the polygons of all sites on a map. 
...
# Wrapper for performing the RDA and reading the output
rda_wrapper <- function (response, explanatory, random_geo_pco=NULL) {
#' This function computes a spatially constrained RDA for all samples in random_geo_pco
#' @param response: the repsonse variable
#' @param explanatory: the explanatory variable 
#' @param random_geo_pco dbmems of all random spatial point patterns
...
# Wrapper function for permuting the RDA
...
```{r test_rda}
# Perform the RDA
#rda_results <- rda_wrapper(factors$genetics, factors$grammar)
rda_results_sp <- rda_wrapper(factors$genetics, factors$grammar, factors$geo)
```


```{r visualize_rda}

# Define a theme for the density plots
...
#' @return the density plot  
  ...
  s <- s + annotate("text", x=peak_obs_x+0.05, y=peak_obs_y+1, label="Observed", color="red", size=3)
  s <- s + annotate("text", x=peak_perm_x+0.05, y=peak_perm_y+1, label="Random permutations", color="blue", size=3)
  s <- s + labs(title=paste("Predicting ", ex, "with ", re, sep=" "),
...
```{r}
#undebug(density_plot)
density_plot(rda_results, "r2_adj")
#debug(spatial_density_plot)
spatial_density_plot(rda_results_sp, "r2")
```


```{r}

# Visualize those spatial patterns for which the drop in R squared is the largest

map_low_r2 <- function(sp_rda_res, rand_points, lang_poly, languages, lang_labels, r2_type, thr){
#' This function plots the spatial locations of models for which removing the effects of space resulted in a low (adjusted) r rsquared
...
#' @return the plot of the mapped random points 
  ...
  # get locationscorresponding to low ids
  ...
  m <- m + geom_text(data=lang_labels,
                       aes(x=long, y=lat, label=nam_label, vjust=-1.1, hjust = .3), size=2)
  m <- m + theme_map()
  #m <- m + labs(title=paste("Locations yielding the largest drop in correlation between", 
  #                          ex, "and", re, sep=" "),
  #              subtitle=paste(label, "in the lowest", thr,"percentile", sep=" "))
  return(m)
}  
...
# Iterate over all combinations and perform RDA for each
...
# Rename the list entries with the names of the explanatory variable and the response

names(rda_all) <- sapply(rda_all, function (q){
  return (paste(q$response, q$explanatory, sep="_"))})

names(rda_sp_all) <- sapply(rda_sp_all, function (q){
  return (paste(q$sample_1$response, q$sample_1$explanatory, sep="_"))})



# Genetics --> Grammar
spatial_density_plot(rda_sp_all$genetics_grammar, "r2")
#spatial_density_plot(rda_sp_all$genetics_grammar, "r2_adj")

# Genetics --> Phonology
spatial_density_plot(rda_sp_all$genetics_phonology, "r2")
#spatial_density_plot(rda_sp_all$genetics_phonology, "r2_adj")

# Grammar --> Music
spatial_density_plot(rda_sp_all$grammar_music, "r2")
#spatial_density_plot(rda_sp_all$genetics_music, "r2_adj")

# Music --> Grammar
spatial_density_plot(rda_sp_all$music_grammar, "r2")
#spatial_density_plot(rda_sp_all$music_grammar, "r2_adj")
...
# Music --> Grammar
locations_low_1 <- map_low_r2(rda_sp_all$music_grammar, geo_random_points, 
                            geo_polygons_map, languages_1, lang_labels, "r2", 0.05)
locations_low_2 <- map_low_r2(rda_sp_all$music_grammar, geo_random_points, 
                            geo_polygons_map, languages_2, lang_labels, "r2", 0.05)

locations_low <- plot_grid(ggplotGrob(locations_low_1), ggplotGrob(locations_low_2))

ggsave('locations_low_mus_gramm.pdf', plot=locations_low, device = 'pdf', path = 'C:/Users/ranacher/Desktop',
  scale = 1, width = 10, height = 10, dpi=300)
