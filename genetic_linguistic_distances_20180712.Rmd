...
trim_data <- function(data.list, trim.to=siberia_metadata_all, extra.coverage=.5) {
...
flatten <- function(data.list, gg=siberia_metadata) {
  df.list <- lapply(seq_along(data.list), function(v) {
    ...
    names(df)[1] <- var.name
    return(select(df, UULID, everything()))
    })
  df.flat <- Reduce(function(x,y) full_join(x, y, by='UULID'), df.list)
